...
module.exports = {
  name: 'sb',
  aliases: ['soundboard', 'sound'],
  description: 'Enable or disable soundboard in a voice channel. Requires Manage Channels permission.',
  async execute(message, args, client, db) {
    ...
    if (args.length === 0) {
      return sendReply('Please specify `on` or `off`. Usage: `+sb [on/off]`', true);
    }
    ...
    try {
      ...
      // Note: Discord API doesn't have a direct soundboard setting for voice channels
      ...
      const successText = new TextDisplayBuilder().setContent(
        `# 🔊 Soundboard ${statusText.charAt(0).toUpperCase() + statusText.slice(1)}\n\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
        `**📺 Channel:** ${voiceChannel.name}\n` +
        `**🔊 Status:** ${statusEmoji} ${statusText.charAt(0).toUpperCase() + statusText.slice(1)}\n` +
        `**👤 By:** <@${message.author.id}>\n\n` +
        `> Soundboard features are now **${statusText}** for this channel.\n` +
        `> Audio capabilities have been updated successfully.`
      );
      ...
      console.error('Error managing soundboard:', error);
      return sendReply('Failed to manage soundboard. Check my permissions and try again.', true);
    }
  },
}; 