...
module.exports = {
  ...
  async execute(message, args, client, db) {
    ...
    db.get(
      `SELECT verified_role, verified_female_role, unverified_role, verif_logs_channel FROM verification_settings WHERE server_id = ?`,
      [guildId],
      async (err, settingsRow) => {
        ...
        const { verified_role, verified_female_role, unverified_role, verif_logs_channel } = settingsRow;
        ...
        db.all(`SELECT role_id FROM verificators WHERE server_id = ?`, [guildId], async (err, rows) => {
          ...
          if (target.roles.cache.has(verified_role) || target.roles.cache.has(verified_female_role)) {
          ...
          try {
            ...
            // Add both roles
            await target.roles.add([verified_female_role]);
            ...
            if (logChannel) {
              const logText = new TextDisplayBuilder().setContent(`## ✅ User Verified (Girl)\n<@${target.id}> has been verified by <@${message.author.id}>`);
