...
module.exports = {
  name: 'remove',
  aliases: ['removeperm', 'revokeperm'],
  description: 'Remove permissions from a user or role in the current channel. Requires Admin or Manage Channels permission.',
  async execute(message, args, client, db) {
    ...
    if (args.length === 0) {
      return sendReply('Please provide a user ID, mention, or role ID to remove permissions from.', true);
    }
    ...
    try {
      ...
      if (!currentPerms) {
        return sendReply('No custom permissions found for this target in this channel.', true);
      }
      ...
      return sendReply('Failed to remove permissions. Please check my permissions and role hierarchy.', true);
    }
  },
}; 